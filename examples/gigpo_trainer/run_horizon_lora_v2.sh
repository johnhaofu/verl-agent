#!/bin/bash
# Horizon v2: GiGPO + LoRA + Compiler-OPD process rewards.
#
# Differences from v1 (run_horizon_lora.sh):
#   - opd_step_reward = 0.1   (per-fix-call credit, capped at 2 calls)
#   - max_ckpt_to_keep = 2    (prevent disk-full crash that killed v1@step40)
#   - experiment_name v2      (separate checkpoint dir)
#
# Per paper §3.1 OPD:
#   r_t^step = +0.1 if action == fix AND fix_count <= 2
#   r_T = 1.0 (compile pass) | 0.0 (compile fail) | -0.1 (invalid action)
#   Total max episode reward = 1.0 + 2*0.1 = 1.2
...
python -m verl.trainer.main_ppo \
    ...
    +env.horizon.opd_step_reward=0.1 \
    +env.horizon.opd_max_credited_validates=2 \
    env.resources_per_worker.num_cpus=$num_cpus_per_env_worker \
    trainer.critic_warmup=0 \
    trainer.logger=['console'] \
    trainer.project_name='verl_agent_horizon' \
    trainer.experiment_name='gigpo_qwen3_4b_lora_horizon_v2_opd' \
