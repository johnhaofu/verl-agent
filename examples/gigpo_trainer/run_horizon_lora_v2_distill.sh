#!/bin/bash
# Horizon v2-distill: GiGPO + LoRA + Best-Trajectory Distillation.
#
# Idea (Tülu 3 / OLMo 3 "on-policy SFT"):
#   - GiGPO loss as before (group-relative advantage on episode reward)
#   - PLUS: SFT loss on tokens of WINNING trajectories (reward >= 0.5)
#   - Total loss = pg_loss + distill_alpha * sft_loss   (alpha=0.1)
#
# Why this is better than OPD reward shaping:
#   - No hand-designed step rewards → no Goodhart's Law / hack risk
#   - SFT only credits already-successful trajectories, never intermediate behavior
#   - Failed groups still produce 0 SFT signal (no fake reward for retrying)
#
# Reward function unchanged from v1 (binary + invalid action penalty).
# This is purely a TRAINER-side change.
...
python -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='gigpo_qwen3_4b_lora_horizon_v2_distill' \
